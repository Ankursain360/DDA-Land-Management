...
namespace AcquiredLandInformationManagement.Controllers
{
    public class UnderSection22DetailsController : BaseController
    {
        ...
        {
            bool IsValidpdf = CheckMimeType(undersection22);
            try
            {
        ...
        public async Task<IActionResult> Undersection22List([FromBody] Undersection22SearchDto model)
        {
            var result = await _undersection22Service.GetAllUndersection22List(model);
       ...
       public bool CheckMimeType(Undersection22 undersection22)
        {
            bool Flag = true;
            string fullpath = string.Empty;          
            string extension = string.Empty;
            DocumentFilePath = _configuration.GetSection("FilePaths:US22:DocumentFIlePath").Value.ToString();
            IFormFile files = undersection22.DocumentIFormFile;
            if (files != null)
            {
                extension = System.IO.Path.GetExtension(files.FileName);
                string FileName = Guid.NewGuid().ToString() + "_" + files.FileName;
