...
function onChange(id) {
    ...
    HttpGet(`/InternalKycForm/GetBranchList/?propertyTypeId=${id}`, 'json', function (response) {
...
$("#ZoneId").change(function () {
    ...
    var ids = $(this).val();
    if (ids) {
        HttpGet(`/InternalKycForm/GetLocalityList/?zoneid=${ids}`, 'json', function (response) {
            var html = '<option value="">Select</option>';
            for (var i = 0; i < response.length; i++) {
                html = html + '<option value=' + response[i].id + '>' + response[i].name + '</option>';
            }
