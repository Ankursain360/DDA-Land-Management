	...
	$("form input,select,textarea").each(function ()
	{
		...
		if ($.trim($(this).val()).length == 0)
		{
			...
			if (!isFormValid) {
				...
				$('#' + FieldId).remove();
				$("#" + FieldId).fadeIn(250);
				$("<span class='help-block' id='" + FieldId + "'>Please Enter Valid Email-Id</span>").insertAfter($(this).parent().closest('div').find("span[class='text-danger lb-sm field-validation-valid']"))
				$(this).parent().closest('div').addClass('has-error');
				isFormValid = false;
			}
			else {
			...
			if (!isFormValid) {
				...
				$('#' + FieldId).remove();
				$("#" + FieldId).fadeIn(250);
				$("<span class='help-block' id='" + FieldId + "'>Passwords must contain at least eight characters, including uppercase, lowercase letters, special character and numbers.</span>").insertAfter($(this).parent().closest('div').find("span[class='text-danger lb-sm field-validation-valid']"))
				$(this).parent().closest('div').addClass('has-error');
				isFormValid = false;
			}
			else {
			...
			if ($(this).attr("min-limit") != undefined && $(this).attr("min-limit") != null && $(this).attr("min-limit") != '') {
				if (parseInt($(this).attr("min-limit")) > $(this).val().length && $(this).val().length > 0) {
					...
					$('#' + FieldId).remove();
					$("#" + FieldId).fadeIn(250);
					$("<span class='text-danger lb-sm' id='" + FieldId + "'>Please Enter Minimum " + parseInt($(this).attr("min-limit")) + " Characters</span>").insertAfter($(this).parent().closest('div').find("span[class='text-danger lb-sm field-validation-valid']"));
					$(this).parent().closest('div').addClass('has-error');
					isFormValid = false;
				}
				else {
