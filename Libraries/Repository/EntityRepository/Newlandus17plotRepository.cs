...
{
   public class Newlandus17plotRepository : GenericRepository<Newlandus17plot>, INewlandus17plotRepository
    {

        public Newlandus17plotRepository(DataContext dbContext) : base(dbContext)
        {

        }
        public async Task<PagedResult<Newlandus17plot>> GetPagedUS17Plot(Newlandus17plotSearchDto model)
        {
            var data = await _dbContext.Newlandus17plot
                                  .Include(x => x.Village)
                                  ...
                                  .GetPaged<Newlandus17plot>(model.PageNumber, model.PageSize);
            ...
            if (SortOrder == 1)
            {
                switch (model.SortBy.ToUpper())
                {
                    ...
                    case ("NOTIFICATION"):
                        data = null;
                        data = await _dbContext.Newlandus17plot
                                               .Include(x => x.Village)
                                               .Include(x => x.Notification)
                                               .Include(x => x.Khasra)
                                               .Where(x => (string.IsNullOrEmpty(model.locality) || x.Village.Name.Contains(model.locality))
        ...
        {
            return await _dbContext.Newlandus17plot
                                   .Include(x => x.Village)
                                   .Include(x => x.Khasra)
                                   .Include(x => x.Notification)
                                   .ToListAsync();
        }
        public async Task<List<Newlandus17plot>> GetAllUS17PlotList(Newlandus17plotSearchDto model)
        {
            var data = await _dbContext.Newlandus17plot
                                  .Include(x => x.Village) 
                                  .Include(x => x.Notification)
