using Dto.Common;

namespace Dto.Master
{
  public  class LandInventoryUnverifiedListDto
    {
        //public int Id { get; set; }
        //public string InventoriedIn { get; set; }
        //public string PlannedUnplannedLand { get; set; }
        //public string ClassificationofLand { get; set; }
        //public string Department { get; set; }
        //public string Zone { get; set; }
        //public string Division { get; set; }
        //public string Colony { get; set; }
        //public string Sector { get; set; }
        //public string Block { get; set; } 
        //public string Pocket { get; set; }
        //public string PlotNo { get; set; }
        //public string PrimaryListNo { get; set; }
        //public string Address { get; set; }
        //public string AreaUnit { get; set; }
        //public string TotalArea_sq_mt { get; set; }
        //public string TotalArea_in_sq_mt { get; set; }
        //public string Encroachment { get; set; }
        //public string Protection_land { get; set; }
        //public string AreaCovered_sq_mtr { get; set; } 
        //public string Dimension { get; set; }
        //public string Boundary { get; set; }
        //public string MainLand { get; set; }
        //public string SubLand { get; set; }
        //public string LitigationStatus { get; set; }
        //public string BuiltUp { get; set; }
        //public string TotalArea_sq_mt { get; set; }
        //public string TotalArea_in_sq_mt { get; set; }
        //public string KhasraNo { get; set; }

        public int Id { get; set; }
        public string InventoriedIn { get; set; }
        public string PlannedUnplannedLand { get; set; }
        public string ClassificationofLand { get; set; }
        public string Department { get; set; }
        public string Zone { get; set; }
        public string Division { get; set; }
        public string Locality { get; set; }
        public string KhasraNo { get; set; }
        public string Colony { get; set; }
        public string Sector { get; set; }
        public string Block { get; set; }
        public string Pocket { get; set; }
        public string PlotNo { get; set; }
        public string PrimaryListNo { get; set; }
        public string AddressWithLandmark { get; set; }

        public string AreaUnit { get; set; } 
        public string Bigha { get; set; }
        public string Biswa { get; set; }
        public string Bishwani { get; set; }
        public string TotalArea { get; set; }
        public string TotalAreaSqmt { get; set; }
        public string Encroachment { get; set; }
        public string EncroachmentStatus { get; set; }
        public string EncroachmentArea { get; set; }
        public string BuiltUpInEncroachmentArea { get; set; }
        public string Vacant { get; set; }
        public string ActionOnEncroachment { get; set; }
        public string EncroachemntDetails { get; set; }
        public string ProtectionOfLand { get; set; }
        public string AreaCovered { get; set; }
        public string Dimension { get; set; }
        public string BoundaryRemarks { get; set; }
        public string BuiltType { get; set; }
        public string LitigationStatus { get; set; }
        public string CourtName { get; set; }
        public string CaseNo { get; set; }
        public string OppositeParty { get; set; }
        public string LitigationStatusRemarks { get; set; }
        public string GeoReferencing { get; set; }
        public string Remarks { get; set; }

    }
}
